...
def validate_upload(name, raw):
    ...
    try:
        ...
        raise ValueError("Audio decoding failed.") from e
    if info.duration < 0.2 or info.duration > 30:
        raise ValueError("Audio must be between 0.2 and 30 seconds.")
    if info.samplerate < 8000 or info.channels not in (1, 2):
        raise ValueError("Unsupported sampling rate or channel count.")
    return info
...
def quality(y):
    ...
    if rms < 0.01:
        return "Unusable", {"rms": rms, "clipping": clipping, "noise": noise}
    if clipping > 0.03 or rms < 0.03:
        return "Poor", {"rms": rms, "clipping": clipping, "noise": noise}
    if noise > 0.12:
        return "Acceptable", {"rms": rms, "clipping": clipping, "noise": noise}
